...
public class ReadExcelUtil implements ReadExcelUtilInterface {
	...
	public List<List<Object>> readExcel(File file,Integer FirstRowNum,Integer LastCellNum) throws IOException {
		String fileName = file.getName();
		String extension = fileName.lastIndexOf(".") == -1 ? "" : fileName
				.substring(fileName.lastIndexOf(".") + 1);
		if ("xls".equals(extension)) {
			return read2003Excel(file,FirstRowNum,LastCellNum);
		} else if ("xlsx".equals(extension)) {
...
//			System.out.println("");
	...
	public List<List<Object>> read2003Excel(File file,Integer FirstRowNum,Integer LastCellNu) throws IOException {
		List<List<Object>> list = new LinkedList<List<Object>>();
...
//		for (int i = sheet.getFirstRowNum(); i <= sheet.getPhysicalNumberOfRows(); i++) {
		...
		for (int i = FirstRowNum; i <= sheet.getPhysicalNumberOfRows(); i++) {
			row = sheet.getRow(i);
			if (row == null) {
				continue;
			}
			...
			if(LastCellNu == null ) {
				//读取的列数量
				LastCellNu = (int) row.getLastCellNum();
			}
			for (int j = row.getFirstCellNum(); j <= LastCellNu; j++) {
				cell = row.getCell(j);
				if (cell == null) {
					linked.add("");
					continue;
